'''
Random class was taken from https://github.com/MostAwesomeDude/java-random.
...
for _ in range(n):
    seed_a, seed_b = [int(i) for i in input().split()]
    stem = [int(input()) for _ in range(10)]
    counter = 0
    for q in range(seed_a, seed_b + 1):
        counter = counter + 1
        var.setSeed(q)
        for i in range(10):
            if stem[i] != var.nextInt(1000):
                break
        if i == 9:
            print(str(q), ' '.join(str(s) for s in [var.nextInt(1000) for _ in range(10)]))
            break
